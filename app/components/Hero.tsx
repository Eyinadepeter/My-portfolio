...
const CodeEditorAnimation = () => {
  ...
  useEffect(() => {
    ...
    backgroundImage.src = "/images/editor-background.jpg"; // ✅ Use your actual image path

    backgroundImage.onload = () => {
      requestAnimationFrame(animate); // Start animation after image loads
    };
    ...
    const codeLines = [
      { text: "// Mizetech Portfolio", color: colors.comment },
      ...
      {
        text: "  const [projects, setProjects] = useState([]);",
      ...
      { text: '      <Header title="Eyinade Peter" />', color: colors.function },
      { text: "      <Hero />", color: colors.function },
      { text: "      status: Available for exciting projects", color: colors.function },
      ...
      for (let i = 0; i < Math.min(currentLine + 1, codeLines.length); i++) {
        ...
        if (i < currentLine && i < codeLines.length) {
          ...
          if (codeLines[i].text.startsWith("  ")) {
            indentLevel =
              Math.floor(codeLines[i].text.match(/^\s+/)?.[0].length || 0) / 2;
          }

          ctx.fillText(
            codeLines[i].text,
            startX + indentLevel * tabSize,
            startY + i * lineHeight
          );
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 font-display">
            <span className="text-white">
              <TypingAnimation text="I'm Peter Olasunkanmi Eyinade" />
            </span>
          </h1>
          ...
          >
            Building modern web applications with Next.js, React, and serverless
            technologies.. Transforming ideas into innovative digital
          ...
          >
            <a
              href="https://www.linkedin.com/in/eyinade-peter/"
            ...
            <a
              href="mailto:eyinadepeter2@gmail.com"
              className="p-3 rounded-full bg-slate-800/80 backdrop-blur-sm border border-slate-700 hover:bg-indigo-600 hover:border-indigo-500 transition-all duration-300"
            ...
            <a
              href="https://github.com/Eyinadepeter"
              className="p-3 rounded-full bg-slate-800/80 backdrop-blur-sm border border-slate-700 hover:bg-indigo-600 hover:border-indigo-500 transition-all duration-300"
